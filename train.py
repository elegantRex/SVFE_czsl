...
# 定义训练函数，参数为model，train_dataset，test_dataset，config
# 遍历训练数据集，计算损失，更新模型参数，计算训练损失
        if (i + 1) % config.save_every_n == 0:
            torch.save(model.state_dict(), os.path.join(config.save_path, f"{config.fusion}_epoch_{i}.pt"))

# 定义验证函数，参数为model，val_dataset，test_dataset，config
# 计算损失，计算验证结果，如果config.best_model_metric为best_loss，则比较损失，如果小于best_loss，则保存模型，如果config.best_model_metric为best_acc，则比较准确率，如果大于best_metric，则保存模型
...
# 定义训练结束函数，参数为model，config
# 如果config.save_model为True，则保存模型
...
if __name__ == "__main__":
    ...
    model = SVFE(config, attributes=attributes, classes=classes, offset=offset).cuda()
