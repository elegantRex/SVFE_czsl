...
#     # aasd----(X = 0.5 * X + 0.5 * X.mean(dim=1, keepdim=True)
...
# a = BiLevelRoutingAttention_nchw()
# ml = BiLevelRoutingAttention_nchw(dim=d_model, num_heads=n_head, attn_backend='torch')
...
class FusionTextImageBlock(nn.Module):
    def __init__(self, width_img: int, width_txt: int, attributes: int, classes: int, layers: int, attn_mask: torch.Tensor = None, context_length: int = 8, fusion: str = "BiFusion"):
        ...
        # self.Biformer_img = BiLevelRoutingAttention_nchw(width_img, width_img//64, attn_mask)
        # self.Biformer_txt = BiLevelRoutingAttention_nchw(width_txt, width_txt//64, attn_mask)
        
        # self.a = BiLevelRoutingAttention_nchw(dim=d_model, num_heads=n_head, attn_backend='torch')
        
        self.Biformer_img = BiLevelRoutingAttention_nchw(dim = width_img, num_heads = width_img//64, attn_backend = 'torch')
        self.Biformer_txt = BiLevelRoutingAttention_nchw(dim = width_txt, num_heads = width_txt//64, attn_backend = 'torch')
